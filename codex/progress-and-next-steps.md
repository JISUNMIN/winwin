...
- 회원가입 화면에 비밀번호 확인 입력과 프론트 검증 추가
- 예약 플로우를 `예약금 계좌이체 -> 고객 입금 알림 -> 파트너 입금 확인 후 확정` 구조로 변경
- 개발용 빠른 전환 UI는 운영 빌드에서 기본적으로 숨겨지는 상태를 유지
- Android Play Store 업로드용 `eas.json`과 EAS build/submit 스크립트 추가
- Android 패키지명/버전코드를 `android/gradle.properties`로 분리
- Android 전용 릴리즈 메타데이터 점검 스크립트 추가
- Play Store 업로드 가이드 문서 추가
...
4. 수동 QA
   고객과 파트너 계정으로 채팅, 이미지, 일정, 예약 요청, 예약금 입금 알림, 예약 확정, 상담 종료 확인
5. Android 스토어 빌드
   `npm.cmd run android:build:production`
6. Play Console 첫 업로드
   production `.aab` 수동 등록

## 지금 판단

- 코드 기준으로는 `배포 직전 마감 단계`입니다.
- 하지만 실제 운영 배포 여부는 `env`, `수동 QA`, `Play Console 업로드 준비`를 통과해야 판단할 수 있습니다.
- 특히 예약금은 PG가 아니라 직접 계좌이체 기반입니다.
