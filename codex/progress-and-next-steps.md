...
지금은 고객용과 샵용이 같은 공용 UI를 재사용하고 있으니, 다음에는 샵 화면만의 디테일을 더 분리할 수 있습니다.
...
### 8. 역할별 초기 상태를 더 현실적으로 나누기

지금은 하나의 mock 채팅 데이터를 두 역할이 같이 바라보는 구조입니다.

나중에는:

```text
고객은 예약 요청 카드에서 결제 가능
샵은 일정 선택 카드에서 예약 요청 생성 가능
각 역할에 맞는 초기 알림/상태 표시
```
