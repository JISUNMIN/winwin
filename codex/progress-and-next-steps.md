...
- 전체 API 전환 기준으로 대략 `65~75%` 정도 진행됐습니다.
- `auth`는 `90%+` 수준입니다.
- `post`는 `85~90%` 수준입니다.
- `partner dashboard`는 `70~80%` 수준입니다.
- `consultation/chat`은 이제 `55~65%` 정도까지 올라왔고, 읽기/텍스트/희망 일정/예약 요청/결제 완료까지 기본 플로우가 붙었습니다.

## 지금 남은 큰 작업

- 파트너/고객 상담 목록과 채팅 메시지를 실제 서버 데이터로 완전히 전환하기
- 상담 종료, 상태 변경, unread 규칙 같은 나머지 관리 액션을 실제 API로 저장하기
- 고객용 상담 목록 같은 남은 기본 API를 채우기
- 상담 상태 변경과 목록/채팅 동기화를 실제 백엔드 상태 기준으로 맞추기
