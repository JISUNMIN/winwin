...
같은 채팅을 고객 보기 / 파트너 보기로 전환할 수 있도록 역할 기반 정렬과 액션 분리도 추가했습니다.

고객용 `/chat/[id]`와 파트너용 `/partner/chat/[id]` 라우트도 분리했고, 공용 채팅 화면 컴포넌트로 재사용 구조를 정리했습니다.

파트너용 채팅 화면에서는 상담 상태를 빠르게 볼 수 있도록 상단 요약 카드도 추가했습니다.

파트너용 채팅 화면에서는 희망 일정 카드와 예약 요청 카드로 바로 이동하는 빠른 액션 버튼도 추가했습니다.

예약 진행 상태는 이제 메시지 배열과 분리된 별도 상태 모델로 관리하도록 정리했습니다.

파트너 전용 채팅 헤더에는 예약 상태 배지와 고객 정보/상담 종료 액션도 추가했습니다.

파트너 입장에서 여러 상담을 볼 수 있도록 `/partner` 상담 목록 화면도 추가했고, 여기서 각 파트너 채팅으로 진입할 수 있게 했습니다.
...
파트너 상담 목록 화면에는 뒤로가기 버튼도 추가해서 상세 화면이나 이전 흐름에서 자연스럽게 돌아갈 수 있게 했습니다.

파트너 상담 목록에서 새 공고 등록 화면으로 들어가고, 입력 후 등록 완료 화면까지 가는 기본 공고 등록 흐름도 만들었습니다.
...
파트너 공고 관리에서 `마감` 처리한 공고는 이제 고객용 홈 목록에서 자동으로 빠지도록 연결했습니다.

공고 수정 저장 후에는 공고 관리 화면 상단에 `수정 완료` 안내 배너가 잠깐 보이도록 연결했습니다.

공고 관리 화면에는 이제 파트너 이름, 서비스명, 위치, 지원 조건 기준으로 공고를 바로 찾을 수 있습니다.

메인 화면 현재 위치와 공고 등록 위치 확인 메시지는 이제 좌표 숫자보다 주소 문자열 중심으로 보이도록 정리했습니다.

메인 화면 검색과 내 공고 관리 검색은 이제 글자 입력마다 바로 실행되지 않고, 검색 버튼이나 엔터를 눌렀을 때만 적용됩니다.

공고 등록 화면의 위치 입력은 이제 큰 주소를 직접 치는 대신 `현재 위치 불러오기`로 기본 주소를 채우고, 필요할 때만 `1층 102호` 같은 추가 상세주소를 적는 방식으로 바꿨습니다.

역할과 라우트 이름도 이제 `shop` 대신 `partner` 기준으로 정리해서, 파트너 상담/공고 관리 흐름이 더 넓은 업종에 맞도록 바꿨습니다.

앱 전체에는 이제 `guest / customer / partner` 기준의 mock 인증 상태도 추가해서, 게스트는 홈/상세만 보고 고객 액션과 파트너 화면은 로그인 후에만 진입하도록 가드를 붙였습니다.

고객은 `지원하기`, 고객 채팅, 희망 일정 전송, 예약 결제 흐름을 로그인 후에만 진행할 수 있고, 파트너는 `/partner` 이하 화면을 로그인 후에만 볼 수 있습니다.

웹에서 auth 라우트 반영을 바로 시험하기 어렵던 구간이 있어서, 홈 상단에는 `게스트 / 고객 / 파트너` 역할 전환 버튼도 추가해 현재 역할을 즉시 바꿔볼 수 있게 했습니다.

메인 매칭 카드의 CTA 문구는 이제 `지원하기`가 아니라 `상세보기`로 바뀌었고, 실제 `지원하기`는 상세 화면 하단의 고객 액션으로만 남겼습니다.

파트너 전용 진입은 상세 화면 안의 `...` 메뉴 대신 메인 우측 상단의 작은 `파트너 바로가기` 버튼으로 옮겨서, `상담 목록 / 공고 관리` 이동이 메인에서 바로 보이도록 정리했습니다.

mock auth 역할은 이제 `AsyncStorage`에 저장되어, 앱을 다시 시작해도 마지막으로 선택한 `guest / customer / partner` 상태가 유지됩니다.

권한 가드도 저장된 역할을 복원하기 전에는 잠깐 대기하도록 바꿔서, 앱 시작 직후 파트너 화면이 잘못 `/auth`로 튀지 않게 정리했습니다.

마지막으로 보고 있던 화면 경로도 이제 같이 저장해서, 앱을 다시 시작하면 현재 역할로 접근 가능한 경우 홈이 아니라 마지막 화면으로 다시 복구됩니다.

이때 `/auth` 같은 중간 화면은 저장하지 않고, `partner`가 아닌데 `/partner...`로 복구하려는 경우도 자동으로 막아서 권한 흐름과 충돌하지 않게 했습니다.

보호 화면의 로딩/권한 안내 분기도 이제 `ProtectedRoleScreen` 공용 wrapper로 정리해서, 파트너/고객 보호 화면들이 같은 방식으로 동작하도록 맞췄습니다.

역할을 바꿨는데 현재 보고 있는 화면이 더 이상 접근 불가인 경우에는, 이제 그 역할의 기본 화면으로 바로 정리하고 마지막 저장 경로도 함께 덮어써서 재실행 시 흐름이 꼬이지 않게 했습니다.
...
### 1. 인증 CTA 다듬기는 나중에 다시 검토

지금은 auth 구조와 역할 가드 동작을 먼저 안정화한 상태입니다.

나중에는:

- 홈 상단 역할 버튼과 별도로 게스트용 `고객으로 시작`, `파트너 센터 보기` CTA를 둘지
- `테스트용 빠른 전환` 라벨을 붙여 개발용 버튼 의미를 분리할지
- `/auth` 제목과 역할 카드 액션 문구를 더 제품형 톤으로 다듬을지

를 다시 검토할 수 있습니다.

### 2. 역할별 진입 CTA 다듬기

현재는 홈 상단 역할 버튼과 접근 제한 시점의 `/auth` 페이지를 함께 쓰고 있습니다.

다음에는:

- 홈 상단 역할 버튼을 개발용으로 남길지 결정
- 게스트일 때 `고객으로 시작`, `파트너로 시작` CTA를 별도 카드로 둘지 검토
- `파트너 바로가기` 버튼을 상단 헤더 구조에 더 자연스럽게 녹일지 검토

같이 더 자연스럽게 정리할 수 있습니다.

### 3. 실제 인증 교체를 위한 경계 유지

이번 단계의 context와 role guard는 mock 버전입니다.

나중에 실제 로그인 API가 붙을 때는:

- 토큰/세션 저장
- 회원 정보 조회
- 권한 기반 redirect 정책

으로 바꾸되, 지금 만든 `useAuth`, `useRoleGuard` 경계를 유지하면 교체가 쉬워집니다.
...
지금 바로 꼭 필요한 다음 작업은 `공고 관리 목록 검색 추가`였고, 이 작업은 완료했습니다.
...
- RN/앱 구현 기록은 기존 step 번호 체계를 유지합니다.
- 백엔드 구현 기록은 다음부터 `backend-01-...` 형식의 별도 시리즈로 정리합니다.
- API는 기능 단위로 만들고 바로 RN 화면에 연결합니다.
