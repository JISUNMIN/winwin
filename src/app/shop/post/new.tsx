...
export default function ShopPostNewScreen() {
  const [selectedCategory, setSelectedCategory] = useState<MatchingCategory>('hair');
  ...
  const [detailLocationState, setDetailLocationState] = useState<VerificationState>('idle');
  ...
  const verifyLocation = async () => {
    ...
    try {
      ...
      const message = '위치 확인 중 문제가 생겼어요. 잠시 후 다시 시도해 주세요.';
      setDetailLocationCoordinates(null);
      setDetailLocationState('error');
      setDetailLocationFeedback(message);
    }
  };
  ...
  const handleSubmit = () => {
    ...
    router.push({
      ...
      params: {
        ...
        service: service.trim(),
        location: derivedPublicLocation,
        detailLocation: normalizedDetailLocation,
        locationVisibility:
          locationVisibility === 'summary-only' ? '예약 후 상세 위치 안내' : '처음부터 상세 위치 공개',
        requirementCount: String(requirements.length),
        dateCount: String(selectedDates.length),
